...
class TestIs_valid_king_jump(TestCase):
    def test_is_valid_king_jump(self):
        ...
        self.assertEqual(vd.is_valid_king_jump(move, board), True)
    ...
    def test_is_invalid_king_jump(self):
        king = cb.Piece(cb.PieceType.KING, 1, {"row": 3, "col": 3})
        board = cb.Board()
        board.put_piece(king, {"row": 3, "col": 3})
        ...
        # Jump over own piece
        move["new_pos"] = {"row": 1, "col": 5}
        self.assertEqual(vd.is_valid_king_jump(move, board), False)
